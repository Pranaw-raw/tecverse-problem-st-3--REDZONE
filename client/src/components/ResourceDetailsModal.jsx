    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 animate-fade-in">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-4xl max-h-[92vh] overflow-y-auto shadow-2xl flex flex-col animate-slide-up">
        ...
        <div className="relative h-64 sm:h-72 w-full overflow-hidden bg-slate-950 shrink-0">
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 p-2 rounded-full bg-slate-950/80 border border-slate-700/60 text-slate-300 hover:text-white hover:bg-slate-900 transition shadow-lg"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <div className="absolute bottom-4 left-4 right-4 sm:left-6 sm:right-6">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <span
                className={`px-3 py-1 text-xs font-semibold rounded-xl border glass-badge ${getCategoryBadgeStyle(
                  resource.category
                )}`}
              >
                {resource.category}
              ...
              <span
                className={`px-3 py-1 text-xs font-semibold rounded-xl border glass-badge capitalize flex items-center gap-1.5 ${getStatusBadgeStyle(
                  resource.status
                )}`}
              >
              ...
              <span className="px-3 py-1 text-xs font-medium rounded-xl bg-slate-900/80 border border-slate-700/60 text-slate-200 glass-badge flex items-center gap-1.5">
                <Users className="w-3.5 h-3.5 text-indigo-400" /> Capacity: {resource.capacity}
              </span>
            </div>
        ...
        <div className="p-5 sm:p-7 space-y-6 flex-1">
          ...
          <div>
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">
              Facility Description & Specifications
            </h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800">
              <h4 className="text-xs font-bold text-indigo-400 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                <Sparkles className="w-4 h-4" /> Hardware & Amenities
              </h4>
            ...
            <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800">
              <h4 className="text-xs font-bold text-amber-400 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                <BookOpen className="w-4 h-4" /> Usage Rules & Guidelines
              </h4>
              <ul className="space-y-2">
                {resource.rules && resource.rules.length > 0 ? (
                  ...
                  <li className="text-xs text-slate-500">Standard campus facility guidelines apply.</li>
                )}
              </ul>
            </div>
          </div>
