...
import { Bell, CheckCheck, Clock, CalendarCheck, AlertTriangle, X, Sparkles, Zap } from 'lucide-react';
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-base font-bold text-white flex items-center gap-2">
                  ...
                  {unreadCount > 0 && (
                    <span className="px-2 py-0.5 text-[11px] bg-indigo-600 text-white font-bold rounded-full">
                      {unreadCount} new
                    </span>
                  )}
                </h3>
                <p className="text-xs text-slate-400">Live booking alerts and schedule reminders</p>
              </div>
            </div>
            <button
          ...
          <div className="p-3.5 bg-gradient-to-r from-amber-500/10 via-indigo-500/10 to-transparent border-b border-slate-800/80 flex items-center justify-between">
            <div className="text-xs text-slate-300">
              <span className="font-semibold text-amber-400 flex items-center gap-1">
                <Zap className="w-3.5 h-3.5" /> Fast Test Alert
              </span>
              Dispatch test schedule reminder
            </div>
            <button
              onClick={triggerTestReminder}
              className="px-3 py-1.5 text-xs font-semibold bg-amber-500/20 text-amber-300 border border-amber-500/40 rounded-xl hover:bg-amber-500/30 transition active:scale-95 shadow-sm"
            >
              Send Alert ⚡
            </button>
          </div>
            ...
            {notifications.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-8 text-slate-500">
                ...
                <p className="text-xs text-slate-500 mt-1 max-w-xs">
                  You'll be alerted when your bookings are confirmed, modified, or upcoming sessions start.
                </p>
              </div>
            ) : (
              notifications.map((n) => (
