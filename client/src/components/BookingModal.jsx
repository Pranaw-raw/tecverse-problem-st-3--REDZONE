...
export const BookingModal = ({ bookingData, onClose, onSuccess }) => {
  ...
  // Convert initial start/end to datetime-local values
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!user) {
        setErrorMsg('Please sign in to reserve campus facilities.');
      ...
      addToast({
        ...
        message: `Successfully booked "${resource.name}". Access pass is available in My Bookings.`,
        type: 'confirmation',
      });
      ...
      setErrorMsg(err.message || 'Booking conflict or server error. Please choose another time slot.');
      addToast({
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-slide-up">
        ...
        <div className="p-5 border-b border-slate-800 flex items-center justify-between bg-slate-950/80">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Confirm Facility Reservation</h3>
              <p className="text-xs text-slate-400">Atomic instant verification & conflict prevention</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 bg-slate-950/50 border-b border-slate-800/80 flex items-center gap-4">
          <img
            src={
              resource.image_url ||
            ...
            alt={resource.name}
            className="w-16 h-16 rounded-2xl object-cover border border-slate-700/80 shrink-0 shadow-sm"
          />
          <div className="flex-1 min-w-0">
            <h4 className="text-sm sm:text-base font-bold text-white truncate">{resource.name}</h4>
            <div className="flex items-center gap-1.5 text-xs text-slate-400 mt-1">
              <MapPin className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
              <span className="truncate">{resource.location}</span>
            </div>
            <div className="flex items-center gap-2 mt-1.5 text-[11px] text-slate-300">
              <span className="px-2.5 py-0.5 rounded-md bg-indigo-500/15 text-indigo-300 border border-indigo-500/30 font-medium">
                {resource.category}
              </span>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. AI Research Workshop / Team Project"
            />
          </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1.5">
              Notes & Equipment Requirements (Optional)
            </label>
            <textarea
              rows="2"
              value={purpose}
              ...
              className="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition"
              placeholder="e.g. Projector required, 8 attendees..."
            />
          </div>

          {/* Booked By Pill */}
          <div className="p-3.5 rounded-2xl bg-slate-950/80 border border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
            ...
            <span className="text-[11px] text-emerald-400 font-semibold flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span> Instant Confirmation
            </span>
          </div>
          ...
          <div className="pt-3 border-t border-slate-800 flex items-center justify-end gap-2.5">
            ...
            <button
              ...
              className="px-6 py-2.5 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-500 rounded-xl shadow-lg shadow-indigo-900/40 transition flex items-center gap-2 disabled:opacity-50 active:scale-95"
            >
              {isSubmitting ? (
                <>
