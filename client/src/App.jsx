...
const MainLayout = () => {
  ...
  const [qrModalData, setQrModalData] = useState(null); // { booking, mode: 'display' | 'scan' }
      ...
      <footer className="border-t border-slate-800/80 bg-slate-950/80 py-8 px-4 text-center text-xs text-slate-500">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <span className="font-bold text-slate-300 font-heading">ReserveHub</span>
            <span>•</span>
            <span>HackArena '26 — PS-3 Smart Resource Booking Platform</span>
          </div>
          <div className="flex items-center gap-4 text-[11px] text-slate-400">
            <span>Role: <strong className="text-white capitalize">{user?.role}</strong></span>
            <span>•</span>
            <span className="text-emerald-400">Zero Scheduling Conflicts Guaranteed</span>
          </div>
        </div>
      </footer>
      ...
      {bookingModalData && (
        <BookingModal
          ...
          onSuccess={(newBooking) => {
            ...
            // Optionally open QR ticket pass immediately
